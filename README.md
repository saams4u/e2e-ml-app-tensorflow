...
1. Build image
```bash
docker build -t e2e-ml-app-tensorflow:latest -f Dockerfile .
```
2. Run container
```bash
docker run -d -p 5000:5000 -p 6006:6006 --name e2e-ml-app-tensorflow e2e-ml-app-tensorflow:latest
```

## Directory structure
```
e2e-ml-app-tensorflow/
├── datasets/                           - datasets
...
```
python e2e-ml-app-tensorflow/train.py --overfit
```
...
```
docker run -p 5000:5000 --name e2e-ml-app-tensorflow e2e-ml-app-tensorflow:latest
```

• Get inside container if using `CMD ["/bin/bash"]`
```
docker run -p 5000:5000 -it e2e-ml-app-tensorflow /bin/bash
```
